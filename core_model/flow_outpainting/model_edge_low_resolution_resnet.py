...
class GatedConv2d_ResNet_Block(torch.nn.Module):
    def __init__(self, in_c, in_o, downsample=None):
        ...
        if downsample == "Down":
            norm_downsample = torch.nn.AvgPool2d(kernel_size=5, stride=2, padding=2)
        elif downsample == "Up":
...
class GatedConv2d_Block(torch.nn.Module):
    def __init__(self, in_c, in_o, downsample=None):
        ...
        if downsample == "Down":
            ...
            norm_downsample = torch.nn.MaxPool2d(kernel_size=5, stride=2, padding=2)
        elif downsample == "Up":
            norm_downsample = torch.nn.Upsample(scale_factor=2, mode="bilinear")
        elif downsample:
            norm_downsample = torch.nn.AvgPool2d(kernel_size=5, stride=2, padding=2)
...
class Conv2d_ResNet_Block(torch.nn.Module):
    def __init__(self, in_c, in_o, downsample=None):
        super().__init__()
        bn_noise1 = get_bn_layer(output_sz=in_o)
        bn_noise2 = get_bn_layer(output_sz=in_o)
        ...
        if downsample == "Down":
            norm_downsample = torch.nn.MaxPool2d(kernel_size=7, stride=2, padding=3)
        elif downsample == "Up":
            norm_downsample = torch.nn.Upsample(scale_factor=2, mode="bilinear")
        elif downsample:
            norm_downsample = torch.nn.AvgPool2d(kernel_size=3, stride=2, padding=1)
        ...
        self.ch_out = torch.nn.Sequential(bn_noise2,
                        torch.nn.ReLU())
    ...
    def forward(self, x):
        ...
        return self.ch_out(x_a + x_b)


class Gated_mpinet(nn.Module):
    def __init__(self, in_channels=60, out_channels=38,
        ...
        self.layer1 = GatedConv2d_Block(16, 32, 'Down')
        self.layer2 = GatedConv2d_Block(32, 64, 'Down')
        self.layer3 = GatedConv2d_Block(64, 64)
        self.layer4 = GatedConv2d_Block(128, 32, 'Up')
        self.layer5 = GatedConv2d_Block(64, 32, 'Up')
        self.layer6 = GatedConv2d_Block(48, 32)
        self.layer7 = torch.nn.Conv2d(32, 2, 5, 2, 1)
    ...
    def forward(self, x1):
        x_0 = self.layer0(x1) # [B, 16, H, W]
        x_1 = self.layer1(x_0) # [B, 32, H/2, W/2]
        x_2 = self.layer2(x_1) # [B, 64, H/4, W/4]
        x_3 = self.layer3(x_2) # [B, 64, H/4, W/4]
        x_4 = self.layer4(torch.cat([x_3, x_2], dim=1)) # [B, 32, H/2, W/2]
        x_5 = self.layer5(torch.cat([x_4, x_1], dim=1)) # [B, 32, H, W]
        x_6 = self.layer6(torch.cat([x_5, x_0], dim=1)) # [B, 32, H, W]
        x_7 = self.layer7(x_6)
...
class mpinet(nn.Module):
    def __init__(self, in_channels=60, out_channels=38,
        ...
        self.layer0 = Conv2d_ResNet_Block(self.in_channels, 16)
        self.layer1 = Conv2d_ResNet_Block(16, 32, 'Down')
        self.layer2 = Conv2d_ResNet_Block(32, 64, 'Down')
        self.layer3 = Conv2d_ResNet_Block(64, 64)
        self.layer4 = Conv2d_ResNet_Block(128, 32, 'Up')
        self.layer5 = Conv2d_ResNet_Block(64, 32, 'Up')
        self.layer6 = Conv2d_ResNet_Block(48, 32)
        self.layer7 = torch.nn.Conv2d(32, 2, 5, 2, 1)
...
class vnet(nn.Module):
    def __init__(self, in_channels=60, out_channels=38, batch_size=1, height=480, width=720,
        ...
        self.mpinet_0 = Gated_mpinet(in_channels=in_channels, out_channels=out_channels, flag=flag)
        self.mpinet_1 = mpinet(in_channels=4, out_channels=out_channels, flag=flag)
    ...
    def forward(self, input):
        ...
        out0 = self.mpinet_0(x1_low)
        out0_h = F.interpolate(input=out0, size=(self.H//4, self.W//4),mode='bilinear', align_corners=False)

        x2_low = F.interpolate(input=x1, size=(self.H//4, self.W//4),mode='bilinear', align_corners=False)
        x2 = torch.cat([x2_low[:,:2,:,:], out0_h], dim=1)
        out1 = self.mpinet_1(x2)
        out1_h = F.interpolate(input=out1, size=(self.H, self.W),mode='bilinear', align_corners=False)
        ...
        if self.plot > 10:
            ...
            for i in range(B):
                ...
                image_s_t = torch.from_numpy(image_s).permute(2,0,1).unsqueeze(0).float()
                temp_img = F.grid_sample(image_s_t, grid_large + out1_h[i,...].cpu().permute(1,2,0).unsqueeze(0))
                temp_img = temp_img.data.cpu().squeeze(0).permute(1,2,0).numpy().astype(np.uint8)
                ...
                f_c = out0_h[i, ...].permute(1,2,0).detach().cpu().numpy()
                ...
                pre = out1_h[i, ...].permute(1,2,0).detach().cpu().numpy()
        ...
        return [out0_h, out1_h], [mask, mask_out, mask_devia]
