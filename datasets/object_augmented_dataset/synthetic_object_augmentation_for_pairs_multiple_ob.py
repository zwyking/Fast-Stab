...
class RandomAffine:
    ...
    def transform_image(self, image, do_flip, theta, shear_values, scale_factors, tx, ty):
        ...
        image_t = cv.warpPerspective(image, h, (int(image.shape[1] * scale_factors[1]), int(image.shape[0] * scale_factors[0])), cv.INTER_LINEAR,
    ...
    def transform_mask(self, mask, do_flip, theta, shear_values, scale_factors, tx, ty):
        ...
        mask_t = cv.warpPerspective(mask.numpy(), h, (int(mask.shape[1] * scale_factors[1]), int(mask.shape[0] * scale_factors[0])), cv.INTER_LINEAR,
                                    borderMode=self.border_flag)
