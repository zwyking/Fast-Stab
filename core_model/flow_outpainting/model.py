...
class mpinet_resnet(nn.Module):
    def __init__(self, in_channels=60, out_channels=38,
        ...
        self.layer1 = Conv2d_ResNet_Block(16, 64, 'Down')
        self.layer2 = Conv2d_ResNet_Block(64, 64, 'Down')
        self.layer3 = Conv2d_ResNet_Block(64, 32)
        self.layer4 = Conv2d_ResNet_Block(32, 32, 'Up')
        self.layer5 = Conv2d_ResNet_Block(32, 32, 'Up')
        self.layer6 = Conv2d_ResNet_Block(32, 16)
        self.layer7 = Conv2d_ResNet_Block(16, 2)
...
class vnet(nn.Module):
    def __init__(self, in_channels=60, out_channels=38, batch_size=1, height=480, width=720,
                 start_filts=64, flag=True, save_img=None, checkpoint_path=None):
        super().__init__()
        self.plot = 0
        self.flag = flag
        self.in_channels = in_channels
        ...
        if checkpoint_path != None:
            ...
            for k,v in checkpoint['state_dict'].items():
                mpinet_0_dict['.'.join(k.split('.')[1:])] = v
            self.mpinet_0.load_state_dict(mpinet_0_dict)
    ...
    def forward(self, input, low_shape=None):
        if self.flag:
        ...
        if low_shape == None:
            ...
            x1_low = F.interpolate(input=x1, size=low_shape,mode='bilinear', align_corners=False)
        out0 = self.mpinet_0(x1_low)
        out0_h = F.interpolate(input=out0, size=(self.H, self.W),mode='bilinear', align_corners=False)

        flow_n1 = flow_crop * mask.unsqueeze(1) + out0_h * mask_out.unsqueeze(1)
        x2 = torch.cat([flow_n1, mask_out.unsqueeze(1)], dim=1)
        out1 = self.mpinet_1(x2)

        flow_n2 = flow_crop * mask.unsqueeze(1) + out1 *  mask_out.unsqueeze(1)

        if self.plot > 10 and self.save_img:
            self.plot = 0
            ...
            for i in range(B):
                ...
                image_s_t = torch.from_numpy(image_s).permute(2,0,1).unsqueeze(0).float()
                temp_img = F.grid_sample(image_s_t, grid_large + flow_n2[i,...].cpu().permute(1,2,0).unsqueeze(0))
                temp_img = temp_img.data.cpu().squeeze(0).permute(1,2,0).numpy().astype(np.uint8)
                ...
                f_c = flow_n1[i, ...].permute(1,2,0).detach().cpu().numpy()
                f_c = flow_to_image(f_c)
                axis[i][8].imshow(f_c)
                axis[i][8].set_title("flow_n1")
                ...
                pre = flow_n2[i, ...].permute(1,2,0).detach().cpu().numpy()
                pre = flow_to_image(pre)
                axis[i][10].imshow(pre)
                axis[i][10].set_title("flow_n2")
        ...
        return [out0_h, out1, flow_n1], [mask, mask_out, mask_devia, mask_test]
